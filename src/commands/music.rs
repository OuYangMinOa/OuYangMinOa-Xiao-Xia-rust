...
pub async fn play(
    ...
    let channel_id = voice_state
        .channel_id
        .expect("[*] Grabbing channel_id error");
...
pub async fn skip(ctx: poise::Context<'_, (), Error>) -> Result<(), Error> {
    ...
    match result {
        Ok(_) => {
            println!("[*] skip !!");
            ctx.say("skip").await.expect("[*] send message error");
        }
        Err(_) => {
            println!("[*] skip error !");
            ctx.say("skip error").await.expect("[*] send message error");
        }
    }
    Ok(())
}
...
pub async fn pause(ctx: poise::Context<'_, (), Error>) -> Result<(), Error> {
    ...
    match result {
        Ok(_) => {
            println!("[*] Pause !!");
            ctx.say("pause").await.expect("[*] send message error");
        }
        Err(_) => {
            println!("[*] Pause error !");
            ctx.say("pause error")
...
pub async fn resume(ctx: poise::Context<'_, (), Error>) -> Result<(), Error> {
    ...
    match result {
        Ok(_) => {
            println!("[*] resume !!");
            ctx.say("resume").await.expect("[*] send message error");
        }
        Err(_) => {
            println!("[*] resume error !");
            ctx.say("resume error")
                .await
                .expect("[*] send message error");
        }
    }
